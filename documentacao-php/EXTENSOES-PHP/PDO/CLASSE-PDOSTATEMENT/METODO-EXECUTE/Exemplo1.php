...
/**
 * * EXEMPLO #1 EXECUTE UMA INSTRUÇÃO PREPARADA COM UMA VARIÁVEL E VALOR VINCULADOS. 
*/
...
try {
    ...
    /*Execute uma declaração preparada vinculando uma variável e valor*/
    $primeiraLetraNome = 'C';
...
<?php
        ...
        foreach($resultadosAtores as $row) {
            $a[] = json_encode(array("actor_id" => $row['actor_id'], "first_nam" => $row['first_name'], "last_name" => $row['last_name'], "last_update" => $row['last_update']));
            echo "<tr>
                   <td>{$row['actor_id']}</td>
